...
router.get('/', function(req, res, next) {
    res.render('adminregister', { title: 'Express' ,msg:''});
  });
  router.post('/',checkEmail, function(req, res, next) {
    ...
    if(inputPassword!= repeatPassword){
      res.render('adminregister', { title: 'Express' ,msg:'Password not matched'});
  
    }else{
      inputPassword =bcrypt.hashSync(req.body.inputPassword,10);
    var adminregisterDetails = new adminregisterModule({
      firstname: firstName,
      lastname :lasttName,
      email :email,
      password:inputPassword
    });
    adminregisterDetails.save((err,doc)=>{
      if(err) throw err;
      res.render('adminregister', { title: 'Express' ,msg:'user register successfully'});
